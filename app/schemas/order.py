...
class OrderCreate(BaseModel):
    ...
    def validate_address_selection(self) -> "OrderCreate":
        if self.shipping_address_id is None and self.shipping_address is None:
            raise ValueError("Teslimat adresi veya teslimat adresi kimliği gereklidir.")

        if not self.contract_accepted:
            raise ValueError("Mesafeli Satış Sözleşmesi onaylanmalıdır.")

        if not self.kvkk_accepted:
            raise ValueError("KVKK metni onaylanmalıdır.")
        return self
